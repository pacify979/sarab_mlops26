"""Central paths and constants for the AMR-fleet monitoring.

The dataset is the Microsoft Azure Predictive Maintenance set, used here as a structural proxy for an AMR fleet: 
each machine is a mobile unit emitting multivariate operational telemetry over time, with
...
# Modeling constants
# Info feeding into model: operating state (sensors), recent incidents (errors), who the unit is (metadata)
SENSORS = ["volt", "rotate", "pressure", "vibration"] # motor current, wheel odom, actuator load, chassis vibration
...
# LOOKAHEAD_H hours of the feature timestamp, else 0. This is analog to the AMR
# considering "will this unit fail to complete its task".
